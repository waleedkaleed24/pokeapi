...
const PokemonPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchDetailsPage = async () => {
      ...
      //Set More Image
      setImage(<div className="image flex justify-center items-center bg-gray-400 border w-64 h-64 m-auto rounded-full shadow-xl max-w-md "> 
      {pokeData.sprites.other.dream_world.front_defaul === null ? <img className="w-full  h-full" src={pokeData.sprites.front_default} alt={pokeData.name} /> : <img className="w-full h-full" src={pokeData.sprites.other.dream_world.front_default} alt={pokeData.name} />}</div>)

      //Set Abilities
      setAbilities(pokeData.abilities.map(el => <p className="ability__name inline-block bg-red-300 rounded-full px-3 py-1 text-sm font-semibold text-gray-800 mr-2 mb-2">{el.ability.name.toString()}</p>))
